...
def quality_filter(survey, data):
    ...
    if survey == 'decals_dr7':
        sel = (data[mag_1] < 22.5) & (data[mag_2] < 22.5)
        #sel = True
...
def dered_mag(survey, data):
    ...
    if survey == 'decals_dr7':
    	data = data
        #sel = ((data['SOF_PSF_MAG_CORRECTED_G'] - data['SOF_PSF_MAG_CORRECTED_R']) < 0.4) # 0.2
        #sel = numpy.lib.recfunctions.append_fields(data, [mag_dered_1, mag_dered_2], [data[mag_1], data[mag_2]], usemask=False, asrecarray=True)
    if survey == 'y3_gold_2_0':
