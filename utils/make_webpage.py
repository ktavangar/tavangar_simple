...
outfile = 'reduced_decals_web.html'
plots = os.listdir(save_dir)

################################################################################

candidate_list = fits.read(candidate_list)
candidate_list = candidate_list[(candidate_list['SIG'] > 10)]
#candidate_list = candidate_list[(candidate_list['SIG'] < 10.051)]
...
def create_entry(candidate):
    ...
    if image_file not in plots:
        ...
        query_image.retrieve_image(image_file, ra, dec)
    image = '{}/{}'.format(save_dir, image_file)
    iname = image.strip('.png')
    image_url = "http://legacysurvey.org/viewer?ra={0}&dec={1}&zoom={2}&layer=decals-dr7".format(ra, dec, 11)
    ...
    """.format(name,sig, ra, dec, mod, iname, image_url, image, fname, plot, plot)
